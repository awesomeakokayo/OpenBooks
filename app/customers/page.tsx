...
type CustomersPageProps = { searchParams: Promise<{ search?: string; page?: string }> };
...
export default async function CustomersPage({ searchParams }: CustomersPageProps) {
  ...
  const { search, page: pageValue } = await searchParams;
  ...
  const [customers, totalCount] = await Promise.all([
    prisma.customer.findMany({ where, orderBy: [{ createdAt: "desc" }, { id: "desc" }], skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE }),
    ...
    <div className="mx-auto flex max-w-[1180px] flex-col gap-7">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1.5 text-sm text-plum/55">{totalCount} customer{totalCount !== 1 ? "s" : ""} in your business records.</p>
        </div>
        <Link href="/customers/new" className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-plum px-5 text-sm font-bold !text-white shadow-[0_10px_28px_rgba(80,48,71,0.14)] hover:bg-plum-deep sm:h-11 sm:w-auto"><Plus size={17} className="text-white" /> Add customer</Link>
      </section>
      ...
      {customers.length === 0 ? (
        <div className="rounded-3xl bg-pale-sage p-10 text-center sm:p-14">
          ...
          <h2 className="mt-5 font-heading text-lg font-extrabold">{search ? "No matching customers" : "No customers yet"}</h2>
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-plum/55">{search ? "Try another name or phone number." : "Add your first customer and OpenBooks will keep their contact details, invoices and payment history together."}</p>
          {!search ? <Link href="/customers/new" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-plum px-5 text-sm font-bold !text-white hover:bg-plum-deep sm:h-11 sm:w-auto"><Plus size={16} className="text-white" /> Add customer</Link> : null}
        </div>
      ) : (
        <>
            ...
            <div className="divide-y divide-plum/10">
              {customers.map((c) => (
                <Link key={c.id} href={`/customers/${c.id}`} className="group grid gap-3 px-5 py-4 transition hover:bg-[#F8F8F6] sm:grid-cols-[1.3fr_1fr_1fr_72px] sm:items-center sm:gap-4">
                  <div className="flex min-w-0 items-center gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-pale-sage text-xs font-extrabold text-plum">{c.name[0]?.toUpperCase() ?? "C"}</div><div className="min-w-0"><p className="truncate text-sm font-bold text-plum">{c.name}</p><p className="mt-0.5 text-xs text-plum/40 sm:hidden">Customer record</p></div></div>
          ...
          <div className="flex items-center justify-center gap-3 pt-1">
            {safePage > 1 ? <Link href={pageHref(safePage - 1)} className="inline-flex h-10 items-center rounded-xl border border-plum/10 bg-white px-4 text-sm font-semibold text-plum hover:bg-pale-sage">Previous</Link> : null}
            <span className="text-xs font-semibold text-plum/50">Page {safePage} of {totalPages}</span>
            {safePage < totalPages ? <Link href={pageHref(safePage + 1)} className="inline-flex h-10 items-center rounded-xl border border-plum/10 bg-white px-4 text-sm font-semibold text-plum hover:bg-pale-sage">Next</Link> : null}
          </div>
        </>
      )}
    </div>
