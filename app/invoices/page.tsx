...
type InvoiceStatusValue = typeof STATUSES[number];
...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<{ status?: string; page?: string }> }) {
  ...
  const status = STATUSES.includes((params.status ?? "") as InvoiceStatusValue) ? (params.status ?? "") : "";
  const rawPage = Number.parseInt(params.page ?? "1", 10);
  const page = Number.isFinite(rawPage) && rawPage > 0 ? rawPage : 1;
  const where = { businessId, ...(status ? { status: status as Exclude<InvoiceStatusValue, ""> } : {}) };

  const [invoices, totalCount] = await Promise.all([
    prisma.invoice.findMany({ where, include: { customer: true, items: true }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE }),
    ...
    <div className="mx-auto flex max-w-[1180px] flex-col gap-7">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1.5 text-sm text-plum/55">{totalCount} invoice{totalCount !== 1 ? "s" : ""} in your records.</p>
        </div>
