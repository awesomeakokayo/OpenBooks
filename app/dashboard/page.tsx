...
export default async function DashboardPage() {
  ...
  const metricsCards = [
    { label: "Sales this month", value: money(metrics.monthSales), detail: "Successful payments", icon: WalletCards, tone: "bg-plum text-white", detailClass: "text-pale-sage" },
        ...
        <div className="openbooks-card p-6 sm:p-7">
          <div className="flex items-end justify-between gap-4 border-b border-plum/10 pb-5">
            <div>
              ...
              <h2 className="mt-2 font-heading text-xl font-extrabold">Recent sales</h2>
            </div>
          ...
          {metrics.recentSales.length === 0 ? (
            <div className="openbooks-card-soft mt-5 flex flex-col items-center px-5 py-10 text-center">
              ...
              <h3 className="mt-4 font-heading text-base font-extrabold">Your sales will show up here</h3>
              <p className="mt-1.5 max-w-sm text-sm leading-6 text-plum/50">Create an invoice or record a sale and OpenBooks will keep the activity in one clean timeline.</p>
                ...
                <div key={sale.id} className="flex items-center justify-between gap-4 py-4">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-plum">{sale.description}</p>
                      <p className="mt-0.5 truncate text-xs text-plum/45">{sale.customer?.name ?? "Walk-in customer"}</p>
                    </div>
                  </div>
                  <span className="shrink-0 text-sm font-extrabold text-plum">{money(Number(sale.totalAmount))}</span>
                </div>
              ))}
            </div>
          )}
