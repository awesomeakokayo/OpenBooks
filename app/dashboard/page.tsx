...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1180px] min-w-0 flex-col gap-8 overflow-x-hidden">
      <section className="flex w-full min-w-0 flex-col justify-between gap-5 sm:flex-row sm:items-end">
      ...
      <section className="grid w-full min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {metricsCards.map(({ label, value, detail, icon: Icon, tone, detailClass }) => (
          <div key={label} className={`w-full min-w-0 overflow-hidden rounded-3xl p-6 shadow-[0_12px_32px_rgba(80,48,71,0.05)] ${tone}`}>
            <div className="flex min-w-0 items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-[0.12em] opacity-60">{label}</p>
                <p className="mt-3 break-words font-heading text-3xl font-extrabold tracking-tight">{value}</p>
                <p className={`mt-2 text-xs font-semibold ${detailClass}`}>{detail}</p>
              </div>
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-black/5">
      ...
      <section className="flex w-full min-w-0 flex-col justify-between gap-4 overflow-hidden rounded-2xl border border-plum/10 bg-white p-4 sm:flex-row sm:items-center sm:px-5">
        <div className="min-w-0">
      ...
      {!metrics.isCurrentMonth && metrics.monthSales === 0 && previousMonth ? (
        <div className="flex w-full min-w-0 flex-col justify-between gap-3 overflow-hidden rounded-2xl border border-plum/10 bg-pale-sage px-5 py-4 sm:flex-row sm:items-center">
          <div className="min-w-0">
          ...
          <Link href={`/dashboard?month=${previousMonth}`} className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-xl bg-plum px-4 text-xs font-bold !text-white hover:bg-plum-deep">View {monthLabels[previousMonth]} <ArrowRight size={14} /></Link>
        </div>
      ) : metrics.isCurrentMonth && metrics.monthSales === 0 && previousMonth ? (
        <div className="flex w-full min-w-0 flex-col justify-between gap-3 overflow-hidden rounded-2xl border border-plum/10 bg-pale-sage px-5 py-4 sm:flex-row sm:items-center">
          <div className="min-w-0">
      ...
      <section className="grid w-full min-w-0 grid-cols-1 justify-items-stretch gap-5 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="openbooks-card min-w-0 w-full max-w-full overflow-hidden p-6 sm:p-7">
        ...
        <div className="min-w-0 w-full max-w-full overflow-hidden rounded-3xl bg-plum p-6 text-white sm:p-7">
          <p className="openbooks-eyebrow text-pale-sage">{metrics.isCurrentMonth ? "This month" : selectedLabel}</p>
          <p className="mt-4 break-words font-heading text-3xl font-extrabold tracking-tight">{money(metrics.monthSales)}</p>
          <div className="mt-5 space-y-3 border-t border-white/10 pt-5 text-sm">
            {metrics.isCurrentMonth ? <div className="flex items-center justify-between gap-2"><span className="text-white/55">Today</span><span className="shrink-0 break-all font-bold">{money(metrics.todaySales)}</span></div> : null}
