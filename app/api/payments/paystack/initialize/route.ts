...
export async function POST(req: NextRequest) {
  ...
  const invoice = await prisma.invoice.findUnique({
    where: { publicToken: invoiceToken },
    include: {
  ...
  // Create PENDING payment first for idempotency + tracking
  await prisma.payment.create({
    data: {
      ...
      metadata: { via: "initialize", publicToken: invoice.publicToken } as never,
    },
  });
  ...
  try {
    const data = await initializeTransaction({
      ...
      reference,
      callbackUrl,
      subaccount: invoice.business.paystackSubaccountCode || null,
      metadata: {
        businessId: invoice.businessId,
        invoiceId: invoice.id,
        customerId: invoice.customerId,
