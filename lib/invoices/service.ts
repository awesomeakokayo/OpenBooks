...
import { generatePublicToken, calculateInvoiceTotal } from "./utils";
...
export async function createInvoice(params: {
  ...
  params.items.forEach((it) => {
    ...
    if (it.unitPrice <= 0) throw new Error("Unit price must be > 0");
  });
  ...
  const { subtotal, total } = calculateInvoiceTotal(params.items, params.discount ?? 0);
  const invoiceNumber = await nextInvoiceNumber(params.businessId);
  const publicToken = generatePublicToken();

  // Default payment methods from BusinessPaymentSetting if not provided
  let methods = params.paymentMethods;
  if (!methods || methods.length === 0) {
    const setting = await prisma.businessPaymentSetting.findUnique({
...
export async function transitionInvoiceStatus(invoiceId: string, to: InvoiceStatus) {
  ...
  if (!canTransition(invoice.status, to)) {
    throw new Error(`Cannot transition from ${invoice.status} to ${to}`);
  }
