...
export function WorkspaceNavigation({ businessName, firstName }: { businessName: string; firstName: string }) {
  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-50 hidden h-screen w-[248px] shrink-0 flex-col border-r border-white/10 bg-plum text-white lg:flex print:hidden">
      ...
      <div className="fixed inset-0 z-[60] lg:hidden" hidden={!mobileOpen} aria-hidden={!mobileOpen}>
        ...
        <div className="absolute inset-y-0 left-0 flex w-[min(88vw,320px)] flex-col bg-plum text-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Workspace navigation">
          ...
          <div className="openbooks-nav-scroll-region min-h-0 flex-1"><nav className="openbooks-scrollbar-hidden h-full space-y-1 overflow-y-auto px-3 pb-5" aria-label="Mobile workspace navigation">{navigation.map(({ href, label, icon: Icon }) => { const active = isActivePath(pathname, href); return <Link prefetch key={href} href={href} onClick={() => setMobileOpen(false)} aria-current={active ? "page" : undefined} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pale-sage focus-visible:ring-offset-2 focus-visible:ring-offset-plum ${active ? "bg-white/12 text-white" : "text-white/65 hover:bg-white/6 hover:text-white"}`}><Icon size={17} strokeWidth={1.9} /><span>{label}</span>{active && <ChevronRight className="ml-auto text-pale-sage" size={15} />}</Link>; })}<Link prefetch href="/guide" onClick={() => setMobileOpen(false)} className="mt-4 flex items-center gap-3 rounded-xl border-t border-white/10 px-3 py-4 text-sm font-semibold text-white/65 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pale-sage"><BookOpen size={17} strokeWidth={1.9} /><span>OpenBooks guide</span></Link><LogoutButton compact /></nav></div>
        </div>
      </div>

      <div className="fixed inset-x-0 top-0 z-50 flex min-h-[84px] items-center justify-between border-b border-plum/10 bg-[#F8F8F6]/95 px-4 backdrop-blur-xl lg:hidden print:hidden"><div className="flex min-w-0 items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-plum text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-plum focus-visible:ring-offset-2" aria-label="Open workspace navigation"><Menu size={19} /></button><div className="min-w-0"><p className="truncate font-heading text-sm font-extrabold text-plum">{businessName}</p><p className="text-[11px] font-medium text-plum/45">Business workspace · NGN</p></div></div></div>
    </>
  );
}

export function WorkspaceHeader({ businessName, firstName }: { businessName: string; firstName: string }) {
  return <header className="sticky top-0 z-40 hidden min-h-[84px] items-center justify-between border-b border-plum/10 bg-[#F8F8F6]/95 px-4 backdrop-blur-xl lg:flex lg:px-8 print:hidden"><div className="min-w-0"><p className="truncate font-heading text-sm font-extrabold text-plum">{businessName}</p><p className="text-[11px] font-medium text-plum/45">Business workspace · NGN</p></div><div className="flex items-center gap-3"><Link prefetch href="/business/settings" className="rounded-xl p-2.5 text-plum/50 transition-colors hover:bg-white hover:text-plum focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-plum focus-visible:ring-offset-2" aria-label="Open settings"><Settings size={18} /></Link><div className="h-8 w-px bg-plum/10" /><div className="flex items-center gap-2 rounded-xl border border-plum/10 bg-white px-2.5 py-2"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-pale-sage text-[10px] font-extrabold text-plum">{(firstName[0] ?? "U").toUpperCase()}</div><span className="max-w-[120px] truncate text-xs font-bold text-plum">{firstName}</span></div></div></header>;
}
