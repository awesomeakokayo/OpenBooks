  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <select name="customerId" value={customerId} onChange={(e) => setCustomerId(e.target.value)} required className="flex h-[48px] w-full rounded-[12px] border border-plum/12 bg-white px-4 text-sm text-plum">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => setItems([...items, { description: "", quantity: 1, unitPrice: 0 }])} className="text-xs font-semibold text-terracotta hover:text-plum">+ Add item</button>
        </div>
        {items.map((it, idx) => (
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5"><label className="text-sm font-medium text-plum">Discount (₦)</label><input name="discount" type="number" step="0.01" defaultValue={0} className="flex h-[48px] w-full rounded-[12px] border border-plum/12 bg-white px-4 text-plum" /></div>
        <div className="flex flex-col gap-1.5"><label className="text-sm font-medium text-plum">Due date</label><input name="dueDate" type="date" className="flex h-[48px] w-full rounded-[12px] border border-plum/12 bg-white px-4 text-sm text-plum" /></div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {["BANK_TRANSFER", "CASH", "POS", "PAYSTACK"].map((m) => <label key={m} className="flex items-center justify-between rounded-[12px] border border-plum/10 px-4 py-2.5"><span className="text-sm font-medium text-plum">{m.replace("_", " ")}</span><input type="checkbox" checked={paymentMethods.includes(m)} onChange={() => toggleMethod(m)} className="h-5 w-5 accent-plum" /></label>)}
      </div>
